...
*   when they are in common use over time
*   when the compound is not immediately understandable from the knowledge of each individual compound element
*   so that the dictionary user confer on what other compounds with the same first or second element mean, based on selection of compound words included
...
Similarly, only a limited selection of derivatives (for example, verbal nouns in _\-ing_) are included in the standard dictionaries. In Norwegian it is possible to form ing-forms from most verbs. Thus, you will be able to find the words _sove_ (to sleep) and _sitere_ (to quote, to cite) in _Bokmålsordboka_ and _Nynorskordboka_, and form the derivations _soving_ (sleeping) and _sitering_ (quotation, citation) yourself. Note, however, that there are potential _ing-_ forms that are rarely used, such as _komming_ (coming) and _væring_. (being).
...
If you do not find the word you are looking for, you can check the larger documentation dictionaries [Norsk Ordbok (for Nynorsk and dialect words)](https://norsk-ordbok.no) and [NAOB (for Bokmål)](https://naob.no/).
